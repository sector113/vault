...
func TestPostgreSQL_Repmgr(t *testing.T) {
	...
	db0, runner0, container0ID := testPostgreSQL_Repmgr_Container(t, "psql-repl-0")

	db1, _, _ := testPostgreSQL_Repmgr_Container(t, "psql-repl-1")

	ctx0, cancel0 := context.WithTimeout(context.Background(), 300*time.Second)
	ctx1, cancel1 := context.WithTimeout(context.Background(), 300*time.Second)
	defer cancel0()
	defer cancel1()
	time.Sleep(20 * time.Second)

	_, err := db0.NewUser(ctx0, dbplugin.NewUserRequest{
		Statements: dbplugin.Statements{
			Commands: []string{
	...
	err = testPostgreSQL_Repmgr_AddUser(t, ctx0, db0)
	if err != nil {
		t.Fatalf("no error expected, got: %s", err)
	}

	err = testPostgreSQL_Repmgr_AddUser(t, ctx1, db1)
	if err.Error() != "failed to execute query: ERROR: cannot execute CREATE ROLE in a read-only transaction (SQLSTATE 25006)" {
		t.Fatalf("expected error was not received, got: %s", err)
	}

	err = runner0.Stop(ctx0, container0ID)
	// err = runner0.Restart(ctx0, container0ID)
	if err != nil {
		t.Fatalf("failed to stop container '%s': %s", container0ID, err)
	}

	err = testPostgreSQL_Repmgr_AddUser(t, ctx0, db0)
	if !strings.HasPrefix(err.Error(), "unable to start transaction: failed to connect to `host=127.0.0.1 user=postgres database=postgres`: dial error (dial tcp 127.0.0.1") {
		t.Fatalf("expected error was not received, got: %s", err)
	}
	...
	err = runner0.Restart(ctx0, container0ID)
	if err != nil {
		t.Fatalf("failed to restart container '%s': %s", container0ID, err)
	}
	time.Sleep(20 * time.Second)
...
func testPostgreSQL_Repmgr_Container(t *testing.T, name string) (*PostgreSQL, *docker.Runner, string) {
	envVars := []string{
